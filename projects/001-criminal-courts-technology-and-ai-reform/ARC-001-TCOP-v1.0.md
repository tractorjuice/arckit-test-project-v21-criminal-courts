...
**Key Findings**:
- **Strengths**: The programme has strong strategic foundations — comprehensive stakeholder analysis (ARC-001-STKE-v1.0), 23 architecture principles explicitly aligned to TCoP (ARC-000-PRIN-v1.0), cloud-first strategy (ARC-001-STRAT-v1.0), open standards commitment (Principle 20), and a robust purchasing strategy via Digital Marketplace procurement (ARC-001-SOW-v1.0)
- **Gaps**: No user research conducted yet (Point 1), no accessibility testing (Point 2), no DPIA completed (Point 7), no security assessment (Point 6), no sustainability assessment (Point 12), and no GDS service assessment preparation (Point 13)
- **Critical Actions**: Complete DPIA before any data processing begins; commission accessibility audit during Alpha; conduct user research with all 8 identified personas; complete Secure by Design assessment
...
Five use cases are defined (UC-1 through UC-5) with detailed actor flows, preconditions, and postconditions.
...
- [x] Data residency requirements met (UK/EU) (Principle 10: UK jurisdiction mandated)
...
- [x] Data classification completed (OFFICIAL / OFFICIAL-SENSITIVE documented in Principle 3)
- [ ] Encryption at rest and in transit (mandated in Principle 3, not yet implemented)
...
**Evidence**:
Privacy is well-addressed in the architecture design but no DPIA has been completed:
...
However, no DPIA has been completed. For a programme processing criminal justice data (law enforcement data under DPA 2018 Part 3, criminal offence data, victim/witness personal data including children and vulnerable adults), this is a **critical gap**. DPA 2018 Part 3 has specific requirements for law enforcement processing that differ from standard UK GDPR and must be assessed by specialist legal advice.

The programme explicitly processes special category data and automated decision-making (AI disclosure review), both of which trigger mandatory DPIA requirements under UK GDPR Article 35.

**Privacy Controls**:
- [ ] Data Protection Impact Assessment (DPIA) completed — **CRITICAL GAP**
- [x] Privacy by design principles applied (Principle 10)
- [x] UK GDPR compliance assessed (Principle 10 — designed but not implemented)
- [ ] Data retention policy defined (planned in NFR-D-001, not yet operational)
- [ ] User consent mechanisms implemented (not yet — DPA Part 3 uses different lawful bases)
- [ ] Data subject rights procedures documented (planned in Principle 10)
...
**Gaps/Actions Required**:
- **CRITICAL**: Complete DPIA before any data processing begins (run `/arckit.dpia`). This is **BLOCKING for Alpha progression**
- **HIGH PRIORITY**: Obtain specialist legal advice on DPA 2018 Part 3 (law enforcement processing) — this differs significantly from standard UK GDPR
- **HIGH PRIORITY**: Formalise cross-agency data sharing agreements (DAPF) with all partner agencies before data exchange
- **HIGH PRIORITY**: Engage ICO early, including consideration of ICO Sandbox for AI-related processing
- **MEDIUM PRIORITY**: Define data retention policies per agency requirements (7+ years for audit logs per Principle 7)
...
The programme has identified relevant GOV.UK services and cross-government platforms for potential reuse.
...
**Evidence**:
Data strategy is well-defined in the architecture:
...
**Data Management**:
- [x] Data architecture documented (high-level in ARC-001-STRAT-v1.0 and ARC-001-PLAT-v1.0)
...
- [ ] Data sharing agreements in place (where needed) — **planned but not formalised** (R-011)
- [ ] Data lineage tracked (planned in Principle 9)

**Gaps/Actions Required**:
- **HIGH PRIORITY**: Create comprehensive data model (run `/arckit.data-model`) during Alpha
- **HIGH PRIORITY**: Formalise cross-agency data sharing agreements before any data exchange (linked to Point 7)
...
| 7. Make privacy integral | ❌ Non-Compliant | **Yes — CRITICAL: No DPIA completed for criminal justice data** |
...
**Overall Score**: 5/13 Compliant (6 Partially Compliant, 2 Non-Compliant)

### Critical Issues Requiring Immediate Action

1. **DPIA Not Completed (Point 7)** — The programme will process criminal justice data (law enforcement data under DPA 2018 Part 3, criminal offence data, special category data) including AI-assisted automated processing. DPIA completion is **mandatory before any data processing begins** and is a **BLOCKER for Alpha progression**. Run `/arckit.dpia`.

2. **No Sustainability Assessment (Point 12)** — A £281M, 5-year programme with significant AI compute workloads has no sustainability assessment, carbon footprint estimate, or green procurement criteria. While not a blocker, this will be scrutinised at Digital Spend Control.

3. **No Security Assessment (Point 6)** — Security by Design principle is strong, but no threat model or security assessment exists. For a programme processing OFFICIAL-SENSITIVE criminal justice data, a Secure by Design assessment must be completed before Beta. Run `/arckit.secure`.

### Recommendations

**High Priority**:
- Complete DPIA for all planned data processing activities (Point 7) — **CRITICAL BLOCKER**
- Commission primary user research with all 8 personas during Discovery (Point 1)
- Complete threat model and Secure by Design assessment during Alpha (Point 6)
...
- Create comprehensive data model with governance framework (Point 10)
...
- [ ] Commission DPIA for all AI and cross-agency data processing activities (Point 7 — **CRITICAL**)
- [ ] Obtain specialist DPA 2018 Part 3 legal advice for law enforcement data processing (Point 7)
...
- [ ] Complete Secure by Design assessment (Point 6) — run `/arckit.secure`
...
**Review Schedule**: Next TCoP review in 3 months (2026-05-05) — expected to follow Discovery/Alpha, by which time Points 1, 6, 7, and 12 should show significant improvement.
...
**Generated on**: 2026-02-05 10:30 GMT
**ArcKit Version**: 1.5.0
**Project**: Criminal Courts Technology & AI Reform (Project 001)
**AI Model**: claude-opus-4-6
**Generation Context**: TCoP assessment informed by ARC-000-PRIN-v1.0 (23 principles), ARC-001-REQ-v1.0 (65 requirements, 8 personas), ARC-001-STKE-v1.0 (15 stakeholders), ARC-001-RISK-v1.0 (20 risks), ARC-001-STRAT-v1.0 (architecture strategy, £281M investment), ARC-001-SOW-v1.0 (procurement strategy), and ARC-001-PLAT-v1.0 (platform design). No external documents, previous TCoP assessments, or departmental technology policies were provided.
